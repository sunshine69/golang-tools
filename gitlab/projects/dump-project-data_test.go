...
func TestGetGitlabProject(t *testing.T) {
	...
	ParseConfig()
	git := GetGitlabClient()
	p, _, err := git.Projects.GetProject(1370, nil)
...
func TestCopyGroupVars(t *testing.T) {
	...
	for {
		gA, resp, err := git.Groups.GetGroup(projectIDLookup[groupName], nil)
		u.CheckErr(err, "")
		gVars, _, err := git.GroupVariables.ListVariables(gA.ID, &opt)
