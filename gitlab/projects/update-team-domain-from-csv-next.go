...
func UpdateTeamDomainFromCSVNext(git *gitlab.Client, filename string) {
	...
	u.CheckErr(err, "UpdateTeamDomainFromCSVNext OpenCSV file")
...
func UpdateTeamOneRow(git *gitlab.Client, idx int, row []string) *Team {
	...
	t := TeamNew(teamName)
	if t.GitlabNamespaceId == 0 {
		CreateGitlabTeam(git, &t)
	}
	return &t
}
...
func UpdateTeamDomainFromExelNext(git *gitlab.Client, filename string) {
	...
	lines, err := f.GetRows("Team")
	u.CheckErr(err, "UpdateTeamDomainFromExelNext GetRows")
	for idx, l := range lines {
		if idx == 0 {
			continue
		}
	...
	lines, err = f.GetRows("Team_Domain")
	u.CheckErr(err, "UpdateTeamDomainFromExelNext GetRows")
	for idx, l := range lines {
		if idx == 0 {
			continue
...
func UpdateTeamDomainOneRow(git *gitlab.Client, idx int, l []string) *TeamDomain {
	...
	if !u.Assert(len(ts) == 1, "Team should exists in Team table", false) {
		return nil
	}
	ds := DomainGet(map[string]string{"where": fmt.Sprintf("name = '%s'", l[1])})
	if !u.Assert(len(ds) == 1, "Domain should exists in Domain table", false) {
		return nil
	}
...
func CreateGitlabDomain(git *gitlab.Client, d *Domain) {
	...
	d.GitlabNamespaceId = newGroup.ID
	d.Update()
}
