...
func UpdateGitlabUser(git *gitlab.Client) {
	...
	for {
		...
		for _, _u := range users {
			...
			user.Name = _u.Name
			user.Note = _u.Note
			user.Is_admin = (u.Ternary(_u.IsAdmin, 1, 0))
			user.Organization = _u.Organization
			user.WebUrl = _u.WebURL
			user.Avatar_url = _u.AvatarURL
			user.Projects_limit = _u.ProjectsLimit
			user.Can_create_group = (u.Ternary(_u.CanCreateGroup, 1, 0))
			user.Can_create_project = (u.Ternary(_u.CanCreateProject, 1, 0))
			user.Using_license_seat = (u.Ternary(_u.UsingLicenseSeat, 1, 0))
			user.Two_factor_enabled = (u.Ternary(_u.TwoFactorEnabled, 1, 0))
			...
			user.External = (u.Ternary(_u.External, 1, 0))
			user.Update()
