...
func DisplayTransferProjectConsole(w http.ResponseWriter, r *http.Request) {
	...
	sqlwhere := fmt.Sprintf(`project.namespace_kind = 'group' AND project.labels NOT LIKE '%%personal%%' AND is_active = 1 AND domain_ownership_confirmed = %s AND project.name LIKE '%%%s%%' AND project.pid IN (SELECT p.pid from project AS p, project_domain AS pd, domain AS d WHERE p.pid = pd.project_id AND pd.domain_id = d.gitlab_ns_id AND p.path_with_namespace NOT LIKE 'mirror/%%' AND p.path_with_namespace NOT LIKE 'disabled-microservices/%%' ORDER BY pd.ts) ORDER BY ts LIMIT 25 OFFSET %d`, migrated, searchName, currentOffset)
