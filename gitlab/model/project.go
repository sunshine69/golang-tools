...
func (p *Project) New(path_with_namespace string, update bool) {
	...
	defer dbc.Close()
	tx, err := dbc.Begin()
	u.CheckErr(err, "Project new")
	stmt, _ := tx.Prepare(`INSERT INTO project(path_with_namespace) VALUES(?)`)
	defer stmt.Close()
