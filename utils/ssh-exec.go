...
// Copy local dir to remote. The remotePath does not have to exist, in that case it will be created. The dirname of srcPaths is preserved if
// it points to a directory. It can be a multiple file paths which will be copied to remotePath/<file-name>
// Use ssh exec and setup-project-v2 for compressing and extracting. Requires remote host has cli setup-project-v2
// If remotePath is empty a random tmp dir would be created and value return to be used for the next command
func (s *SshExec) CopyDir(remotePath string, srcPaths ...string) (out string, err error) {
	...
	utar -c -f - {{ range .srcPaths }} {{ . }} {{ end }} | $SSH_CMD {{ .ssh_user }}@{{ .ssh_host }} "cat - | utar -x - -C {{ .remote_path }}"
	`, map[string]any{
...
func (s *SshExec) Exec(commands string) (out string, err error) {
	...
	if len(commandList) > 1 {
		...
		remotePath := Must(s.CopyFile(s.SshExecHost, "", tempF+"/remote-exec-cmd.sh"))
